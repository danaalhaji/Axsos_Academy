const pokémon = Object.freeze([
    ...
    const poisionPkmn = pokémon.filter( p => p.types[0] === "poison" || p.types[1] === "poison" ).map(p => p.name);
    const firstTypePkmn = pokémon.filter( p => p.types[1] === "flying" ).map(p => p.types[0]);
    const countTypePkmn = pokémon.filter( p => p.types[0] === "normal").length;
    console.log(pkmnIds);
    console.log(firePkmn);
    console.log(poisionPkmn);
